import { EmbedBuilder, bold, EmbedFooterOptions } from "@discordjs/builders";
...
export function uptimeEmbed(interaction: ButtonInteraction | ChatInputCommandInteraction): EmbedBuilder {
    return new EmbedBuilder()
    .setTitle(`**Время работы бота: ${Math.floor(interaction.client.uptime / 1000 / 60 / 60)} часов ${Math.floor(interaction.client.uptime / 1000 / 60 % 60)} минут ${Math.floor(interaction.client.uptime / 1000 % 60)} секунд**`)
    .setColor(Colors.Green)
}